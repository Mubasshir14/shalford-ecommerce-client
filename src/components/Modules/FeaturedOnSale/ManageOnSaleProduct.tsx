...
export default function ManageOnSaleProduct() {
  ...
  useEffect(() => {
    const fetchNotOnsale = async () => {
      const toastId = "fetching-products";
      try {
        setLoading(true);
        const data = await getNotOnSaleProduct();
        setOnSale(data?.data || []);
        toast.success("Products fetched successfully", { id: toastId });
      } catch (err: any) {
        const message =
  ...
  const filteredNotOnsaleProducts = onSale.filter((product) =>
    product.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="p-6 space-y-6 font-[Sansita]">
                      ...
                      <td className="px-4 py-2 text-sm capitalize text-gray-700 border-b border-amber-200">
                        <td className="px-4 py-2 text-sm capitalize text-gray-700 border-b border-amber-200">
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                const res = await OnSaleProductHandle(
                                  product._id,
                                  false
                                );
                                toast.success("Remove from on sale!");
                              } catch (error: any) {
                                toast.error(
      ...
      <div>
        ...
        <div className="border-2 border-amber-300 rounded-xl shadow-md bg-amber-50/30 p-6">
          {loading ? (
            <p className="text-sm text-gray-500">Loading products...</p>
          ) : filteredNotOnsaleProducts.length === 0 ? (
            <p className="text-sm text-gray-500">No products found.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse">
                ...
                <tbody>
                  {filteredNotOnsaleProducts.map((product) => (
                    <tr
                      key={product._id}
                      className="hover:bg-amber-50 transition"
                    >
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              toast.success("Product marked as on sale!");
                            } catch (error: any) {
                              toast.error(
                                error.message || "Failed to update product"
                              );
