...
import {
  ...
  deleteNewsLetter,
  getAllNewsLetters,
} from "@/components/Services/NewsLetter";
...
export default function ManageNewsletter() {
  ...
  const handleUpload = async () => {
    ...
    try {
      setLoading(true);
      const newBanner = await createNewsLetter(formData);
    ...
    <div className="p-6 space-y-6 font-[Sansita]">
      ...
      <div>
        <h2 className="text-xl font-bold mb-3 text-amber-600">
          All Uploaded Newsletter
        </h2>
        {newsletter.length === 0 ? (
          <p className="text-sm text-gray-500">No newsletter uploaded yet.</p>
        ) : (
