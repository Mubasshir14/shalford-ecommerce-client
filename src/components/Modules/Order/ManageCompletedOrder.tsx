...
import { ManageCompletedOrderAdminProps } from "@/app/(WithDashboardLayout)/admin/dashboard/completed-order/page";
...
const ManageCompletedOrderAdmin: React.FC<ManageCompletedOrderAdminProps> = ({
  Completed,
}) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      if (!Completed) {
        console.error("Completed status is undefined or empty");
      ...
      setLoading(true);
      try {
        const res = await getStatusBasedOrders(Completed);
...
export default ManageCompletedOrderAdmin;
