...
import { ManageProcessingOrderAdminProps } from "@/app/(WithDashboardLayout)/admin/dashboard/processing-order/page";
...
const ManageProcessingOrderAdmin: React.FC<ManageProcessingOrderAdminProps> = ({
  Processing,
}) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      if (!Processing) {
        console.error("Processing status is undefined or empty");
      ...
      setLoading(true);
      try {
        const res = await getStatusBasedOrders(Processing);
        const data: Order[] = res.data.map((order: any) => ({
          ...order,
          order: {
            ...order.order,
                              ...
                              <motion.button
                                ...
                                onClick={() =>
                                  handleStatusChange(
                                    order.order._id,
                                    "Completed",
                                    "/admin/dashboard/completed-order"
                                  )
                                }
                                className="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white rounded-xl text-xs font-bold shadow-lg transition-all duration-300 border border-amber-400 group-hover:shadow-xl"
                              >
...
export default ManageProcessingOrderAdmin;
