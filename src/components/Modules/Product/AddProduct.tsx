...
import { createProduct } from "@/components/Services/Product";
...
export default function AddProduct() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const message = err?.response?.data?.message?.includes("duplicate")
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div>
          ...
          <select
            ...
            className={`w-full capitalize p-2 border-2 rounded-lg focus:ring-amber-500 focus:outline-none disabled:opacity-50 bg-white ${
        ...
        <Button
          type="submit"
          disabled={
            isSubmitting || sizes.length === 0 || imageFiles.length === 0
          }
